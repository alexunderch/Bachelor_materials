...
{
  ForwardIterator max_elem  = find_if(first, last, pred);
  for (ForwardIterator forward_iter = max_elem; forward_iter != last; ++ forward_iter)
  {
    if (pred(*forward_iter) && *forward_iter > *max_elem && max_elem != forward_iter)
    {
      max_elem = forward_iter;
    }
  }
  return max_elem;
}
