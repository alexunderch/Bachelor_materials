...
public:
  ...
  AirportCounter() {
    for (auto it = 0; it < static_cast<uint32_t>(TAirport::Last_); it++) {
      airports[it] = 0;
    }
  }
  ...
  AirportCounter(TIterator begin, TIterator end) {
    ...
    for(auto it = begin; it != end; ++it) {
      ...
      //cout << static_cast<uint32_t>(TAirport(*it)) << " ";
      //cout << airports[static_cast<uint32_t>(TAirport(*it))]  << " ";
    }
  }
  ...
  Items GetItems() const{
    Items result;
    for (auto it = 0; it < static_cast<uint32_t>(TAirport::Last_); it++) {
      Item i = make_pair(static_cast<TAirport>(it), airports[it]);
      result[it] = i;
    }
    return result;
  }
