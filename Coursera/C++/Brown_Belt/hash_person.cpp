...
struct Address {
  ...
  bool operator==(const Address& other) const {
    return tie(other.city, other.street, other.building) ==
...
struct AddressHasher {
  size_t operator () (const Address& addr) const{
    const size_t coeff = 200017;
    return coeff * coeff * shash(addr.city) + 
           coeff * shash(addr.street) +
           ihash(addr.building); 
  }
  hash<string> shash; 
  hash<int> ihash; 
...
struct PersonHasher {
  size_t operator () (const Person& p) const{
    const size_t coeff = 40017;
    return coeff * coeff * coeff * addr_hash.shash(p.name) + 
           coeff * coeff * addr_hash.ihash(p.height) +
           coeff * dhash(p.weight) + 
           addr_hash(p.address); 
  }
  hash<double> dhash;
