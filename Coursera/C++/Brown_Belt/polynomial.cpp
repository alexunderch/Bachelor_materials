...
class Polynomial {
private:
  // ставить mutable нельзя, так как меняется наблюдаемое состояние объекта
...
public:
  ...
  private:
    ...
    size_t degree_;
  public:
    IndexProxy(Polynomial& poly, size_t degree) : poly_(poly), 
                                                  degree_(degree) {}
    IndexProxy& operator = (const T& to_assign) {
    if (degree_ >= poly_.coeffs_.size()) {
      poly_.coeffs_.resize(degree_ + 1);
    }
    poly_.coeffs_[degree_] = to_assign;
    poly_.Shrink();
    return *this;
    }
