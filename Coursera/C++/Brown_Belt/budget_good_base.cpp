...
//structure to show spent and earned money
struct Stats {
...
struct BulkTaxApplier {
  ...
  //}
  double factor = 1.0;
  static BulkTaxApplier FromPercentage (uint8_t tax_percentage) {
...
public:
  ...
  Stats Collapse(Stats origin, IndexSegment segment) const {
    return {
...
class BudgetManager : public SummingSegmentTree<Stats, BulkLinearUpdater> {
public:
...
struct ComputeIncomeRequest : ReadRequest<double> {
  ...
  double Process(const BudgetManager& manager) const override {
    return manager.ComputeSum(MakeDateSegment(date_from, date_to)).ComputeDiff();
  }
...
struct EarnRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    ...
    manager.AddBulkOperation(date_segment, BulkMoneyAdder{ Stats{ daily_income, 0.0 } });
  }
...
struct PayTaxRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    manager.AddBulkOperation(MakeDateSegment(date_from, date_to), BulkTaxApplier::FromPercentage(tax_percentage));
  }
...
struct SpendRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    ...
    manager.AddBulkOperation(date_segment, BulkMoneyAdder{0.0, daily_outcome});
  }
